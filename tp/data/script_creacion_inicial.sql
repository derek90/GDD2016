...
CREATE PROCEDURE HARDCOR.crear_empresa (@username NVARCHAR(255),
  ...
  BEGIN TRY
    BEGIN TRANSACTION
      ...
      INSERT INTO HARDCOR.Empresa (cod_us, cod_contacto, emp_razon_soc, emp_cuit, emp_ciudad, emp_rubro)
      VALUES (@codigo_usuario, @codigo_contacto, @razon_social, @cuit, @ciudad, @rubro)
  ...
  SELECT @count, P.*
   ...
   WHERE P.estado = 1
     ...
     AND @username <> (SELECT username
                         FROM HARDCOR.Usuario U
                        WHERE U.cod_us = P.cod_us)
   ORDER BY HARDCOR.calcular_peso_visibilidad(cod_visi) DESC
  OFFSET @pagina * @cantidad_resultados_por_pagina ROWS
   FETCH NEXT @cantidad_resultados_por_pagina ROWS ONLY
END
GO
...
EXEC HARDCOR.obtener_empresa 30
SELECT * FROM HARDCOR.Empresa e WHERE e.emp_razon_soc = 'unaEmpresa'
